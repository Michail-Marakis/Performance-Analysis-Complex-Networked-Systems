...
from my_awesome_app.task import Net, set_weights
...
class CustomFedAvg(FedAvg):
    ...
    def aggregate_fit(
        ...
        self.last_round_duration = time.time() - start_time
        ...
        if parameters_aggregated is not None:
            ...
            print(f"[SERVER] [INFO] [ROUND {server_round}] Aggregation completed in {self.last_round_duration:.2f} seconds.")
            ...
            round_results = {
                "round_duration_seconds": round(self.last_round_duration, 2),
                "cumulative_payload_mb": round(self.cumulative_data_transferred_mb, 2),
                "round_payload_mb": round(total_mb, 2)
            }
        
            if metrics_aggregated:
                round_results.update(metrics_aggregated)
                
            self.results_to_save[server_round] = round_results
        
            output_path = r"C:\Users\lenia\Desktop\numenv\nolora\results.json"
            
            with open(output_path, "w") as json_file:
                json.dump(self.results_to_save, json_file, indent=4)
            print(f"[SERVER] [INFO] Results successfully saved directly to: {output_path}")
    ...
    def evaluate(
        ...
        my_results = {
            "loss": loss, 
            "round_duration_seconds": round(self.last_round_duration, 2),
            "cumulative_mb": round(self.cumulative_data_transferred_mb,2),
            **metrics
        }
