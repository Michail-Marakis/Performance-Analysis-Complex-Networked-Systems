...
def Net():
    """Load the base BERT model from Hugging Face for binary classification."""
    ...
    if os.path.exists(weights_path):
        ...
        print("[INFO] [TASK] Model loaded successfully from pytorch_model.bin (Safety Bypass).")
    else:
        raise OSError(f"[INFO] [TASK] No file with weight was found in: {weights_path}")
...
def train(net, trainloader, epochs, lr, device):
    """Train the entire BERT model (Baseline Full FL)."""
...
def test(net, testloader, device):
    """Evaluate the BERT model and return loss and accuracy."""
